...
import HistoryOrderTable from '../HistoryOrderTable.vue'
import type { HistoryOrderListQuery } from '../types'

describe('MmHistoryOrderTable', () => {
  it('normalizes the initial request and renders the history-order contract', async () => {
    const request = vi.fn(async (_query: HistoryOrderListQuery) => ({
      rows: [{
    ...
    const wrapper = mount(HistoryOrderTable, {
    ...
    expect(wrapper.get('[data-mm-component="pro-table"]').attributes('aria-label')).toBe('历史委托数据')
  ...
  it('hides user type and omits it from requests when disabled', async () => {
    const request = vi.fn(async (_query: HistoryOrderListQuery) => ({
      rows: [{ order_id: 802, user_id: 9002, user_type: 1 }],
      total: 1,
    }))
    const wrapper = mount(HistoryOrderTable, {
  ...
  it('supports all filters, server sorting, and page pagination', async () => {
    const request = vi.fn(async () => ({ rows: [{ order_id: 801, user_id: 9001 }], total: 45 }))
    const wrapper = mount(HistoryOrderTable, { props: { request } })
    await flushPromises()
    ...
    wrapper.findComponent({ name: 'MmProTable' }).vm.$emit('update:sort', { key: 'price', order: 'asc' })
    await flushPromises()
    expect(request).toHaveBeenLastCalledWith(
      expect.objectContaining({ order_by: 'price', order_dir: 'asc', page_no: 1 }),
      expect.any(Object),
    )

    wrapper.findComponent({ name: 'MmProTable' }).vm.$emit('update:currentPage', 2)
    await flushPromises()
    expect(request).toHaveBeenLastCalledWith(
      expect.objectContaining({ order_by: 'price', order_dir: 'asc', page_no: 2 }),
      expect.any(Object),
    )
    wrapper.unmount()
  })

  it('supports business slots, detail action, reactive keyword, and exposed reload', async () => {
    ...
    const detail = vi.fn()
    const wrapper = mount(HistoryOrderTable, {
      props: { actions: { detail }, initialKeyword: '', request },
      slots: {
