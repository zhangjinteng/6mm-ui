...
export type ConditionOrderListRequest<Row extends ConditionOrderRow = ConditionOrderRow> = (
  query: ConditionOrderListQuery,
  context: ConditionOrderRequestContext,
) => Promise<ConditionOrderListResult<Row>>

export type ConditionOrderTableColumns<Row extends ConditionOrderRow = ConditionOrderRow> =
  | ProTableColumn<Row>[]
  | ((defaultColumns: ProTableColumn<Row>[]) => ProTableColumn<Row>[])

export type ConditionOrderTableQueryFields =
  | QueryBarField[]
...
export type ConditionOrderTableActionHandler<Row extends ConditionOrderRow = ConditionOrderRow> = (
  row: Row,
  context: ConditionOrderTableActionContext,
) => Promise<void> | void
...
export interface ConditionOrderTableCoreProps<Row extends ConditionOrderRow = ConditionOrderRow>
  extends ConditionOrderTablePresetProps<Row> {
  kind: ConditionOrderKind
}
