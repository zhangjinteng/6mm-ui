...
<template>
  <MmProTable v-bind="{ ...$attrs, ...proTableBindings }" class="mm-funding-change-log-table"
    ...
    <template #cell-frozen_amount="slotProps"><slot name="cell-frozen_amount" v-bind="slotProps"><span class="mm-funding-change-log-table__amount" :class="fundingBalanceChangeClass(slotProps.row.frozen_amount)">{{ formatFundingAmount(slotProps.row.frozen_amount, slotProps.row.currency) }}</span></slot></template>
