...
export type CurrentOrderListRequest<Row extends CurrentOrderRow = CurrentOrderRow> = (
  query: CurrentOrderListQuery,
  context: CurrentOrderRequestContext,
) => Promise<CurrentOrderListResult<Row>>

export type CurrentOrderTableColumns<Row extends CurrentOrderRow = CurrentOrderRow> =
  | ProTableColumn<Row>[]
  | ((defaultColumns: ProTableColumn<Row>[]) => ProTableColumn<Row>[])

export type CurrentOrderTableQueryFields =
  | QueryBarField[]
...
export type CurrentOrderTableActionHandler<Row extends CurrentOrderRow = CurrentOrderRow> = (
  row: Row,
  context: CurrentOrderTableActionContext,
) => Promise<void> | void
