...
export type HistoryOrderSortField = 'created_at' | 'order_face_value' | 'order_id' | 'price' | 'quantity' | 'user_id';
...
export type HistoryOrderListRequest<Row extends HistoryOrderRow = HistoryOrderRow> = (query: HistoryOrderListQuery, context: HistoryOrderRequestContext) => Promise<HistoryOrderListResult<Row>>;
export type HistoryOrderTableColumns<Row extends HistoryOrderRow = HistoryOrderRow> = ProTableColumn<Row>[] | ((defaultColumns: ProTableColumn<Row>[]) => ProTableColumn<Row>[]);
export type HistoryOrderTableQueryFields = QueryBarField[] | ((defaultFields: QueryBarField[]) => QueryBarField[]);
export interface HistoryOrderTableActionContext {
    reload: () => Promise<void>;
}
export type HistoryOrderTableActionHandler<Row extends HistoryOrderRow = HistoryOrderRow> = (row: Row, context: HistoryOrderTableActionContext) => Promise<void> | void;
